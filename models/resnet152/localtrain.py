...
LR = 0.01
clothes_type = 'blouse'  # 7158 images
train_sel = (0, 6158)
val_sel = (6158, 7158)

postfix = 'local_stage2_dense161_'
net1_path = "/home/wuxiaodong/fa/models/resnet152/snapshots/stage2_dense161__epoch40.pth"
...
net2 = localfea.get_localmodel(state_dict=torch.load(net1_path, map_location=lambda storage, loc: storage),num_keypoints=num_keypoints)
net2.cuda()
...
def train2():
    ...
    for batch_idx, (data, label)in enumerate(trainloader):
        ...
        # data.volatile = False
        output2 = net2(data, points=output1.data)

        tmp = Variable(output2.data, requires_grad=False)
        optimizer.zero_grad()

        loss = criterion(label-output1[:, :3*num_keypoints], tmp[:, :3*num_keypoints], visibility=label)
        loss.backward()
        optimizer.step()

        tmp = Variable((label-output1[:, :3*num_keypoints]).data, requires_grad=False)
        optimizer.zero_grad()
        loss = criterion(output2[:, :3*num_keypoints], tmp, visibility=label)
        loss.backward()
        optimizer.step()
...
def test2():
    ...
    for batch_idx, (data, label) in enumerate(valloader):
        ...
        # output2 = net2(data)
        output2 = net2(data, points=output1)
        output2 = output2.cpu().data

        output = output1[:, :2*num_keypoints] + output2
