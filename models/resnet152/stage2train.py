...
def train2():
    ...
    optimizer = torch.optim.SGD([{'params': net2.res152.parameters(), 'lr': LR },
                             {'params': net2.outputs.parameters(), 'lr': LR}, ], momentum=0.9, weight_decay=0.0005,
                            nesterov=True)
    ...
    for batch_idx, (data, label)in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        loss = criterion(output2[:, :3*num_keypoints], label-output1[:, :3*num_keypoints])
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    for epoch in range(300):
        ...
        test2()
        if epoch %20 == 0 :
            torch.save(net2.state_dict(), os.path.join(os.path.dirname(__file__),'snapshots' ,postfix+'_epoch{}.pth'.format(epoch)))
